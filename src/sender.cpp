...
int main(int argc, char *argv[]) {
    ...
    if (argc == 6) {
        ...
        max_buffer_size = MAXLINE * (int) atoi(argv[3]);
        dest_ip = argv[4];
    ...
    while (!read_done) {
        ...
        int seq_count = buffer_size / MAXLINE + ((buffer_size % MAXLINE == 0) ? 0 : 1);
        int seq_num;
            ...
            for (int i = 0; i < window_len; i ++) {
                ...
                if (seq_num < seq_count) {
                    ...
                    if (!window_sent_mask[i] || (!window_ack_mask[i] && (elapsed_time(current_time(), window_sent_time[i]) > TIMEOUT))) {
                        ...
                        data_size = (buffer_size - buffer_shift < MAXLINE) ? (buffer_size - buffer_shift) : MAXLINE;
